...
public class SimpleMapFactory<T extends Map> implements MapFactory<T> {
	...
	public SimpleMapFactory(Class<T> type, Class<? extends T> instanceType) {
		this.type = type;
	...
	public T create() {
		try {
			...
			throw new BaseException(e, "Failed to instantiate a map of type %s: %s", instanceType.getSimpleName(), e.getMessage());
		}
	}
